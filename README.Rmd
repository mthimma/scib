...
4. Install `python-annoy` dependency which is needed by bbknn and scanorama.
...
3. Create the conda environment via miniforge.
...
```{bash, eval=FALSE}
conda install --name scib -c conda-forge python-annoy
```

5. Install the data integration softwares.

```{bash, eval=FALSE}
conda install --name scib -c bioconda -c conda-forge \
 scvi-tools scanpy scpython bbknn anndata scanorama 
```
